...
export const LoanRequestCard = ({ data, image, title, address, loanID, principal, repayment, duration, onClick }) => {
	const getButton = () => {
		if (data.status == "0") {
			return (<Button type="primary" disabled>Cancelled</Button>);
		} else if (data.status == "3") {
			return (<Button type="primary" disabled>Dealed</Button>);
		} else {
			return (<Button type="primary" onClick={() => onClick(data)}>Deal</Button>);
		}
	}
